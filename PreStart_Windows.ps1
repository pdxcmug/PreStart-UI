...
.SYNOPSIS 
	...
	variant also provides the option to backup user data before OSD, maintan direct associations after 
	OSD, and configure the PC as an autologon device using a domain account. 
.PARAMETERS 
    None
.EXAMPLE 
    None 
...
$ScriptBlockRunButton = {
	...
	# Set variable to run USMT if the tickbox is checked
	If ($checkBox1.Checked) { 

	$TSEnv.Value("SMSTS_RunUSMT") = "True"
...
$checkBox1.Text = "In-Place Backup/Restore"
...
This will backup user data via USMT and restore it after the imaging
process is complete.
